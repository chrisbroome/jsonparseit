...
_.extend(Parser.prototype, {
  ...
  objectMembers: function objectMembers() {
    return this.parseList('objectMembers', 'pair', ',');
  },
   ...
   * @param {String} open - The opening token
   * @param {String} close - The closing token
   * @param {String} nestedHandler - The name of the method that handles parsing the nested elements
   * @returns {Object|null}
   */
  parseNested: function parseNested(type, open, close, nestedHandler) {
    var token, node;
    node = {type: type, open: null, close: null, children: null};
    node.open = this.parseTerminal(open);
    if (!this.assertNotEndOfInput(close, '|', nestedHandler)) return null;
    token = this.currentToken();
    if (token.type !== close) node.children = this[nestedHandler]();
   ...
   * @param {String} type - The type of node that this list represents
   * @param {String} memberHandler - The name of the method that handles parsing one element of the list
   * @param {String} separator - The type of node that represents the separator value for elements in the list
   * @returns {Object}
   */
  parseList: function parseList(type, memberHandler, separator) {
    var members = [], token, notDone = true, node = {type: type, value: members};
    do {
   ...
   * @param {String} type - The type of terminal to parse
   * @returns {Object|null}
   */
